...
public class World {
	...
	public LinkedList<Agar>  agarList   = new LinkedList<Agar>();
	...
	public Agar getAgar(String agarName) {
		for (Agar anAgar : agarList) {
			if (anAgar.getName().equals(agarName)) { return anAgar; }
		}
		return agarList.getFirst();
	}
	...
	public double getMaxBulkValue(int soluteIndex) {
		...
		// Farzin 25-1-12: changed to let have system with no bulk
		if(getAllBulkValue(soluteIndex).length>0)
