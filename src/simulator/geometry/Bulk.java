...
public class Bulk {
	...
	public Bulk(Simulator aSim, XMLParser aBulkRoot) {
		...
		for (Element asoluteMarkUp : soluteList) {
			...
			System.out.println("soluteIndex"+soluteIndex);
			...
			//System.out.println("_bulkIsConstant...........678......."+_bulkIsConstant);
			...
			if(!_bulkIsConstant){
				...
				if(!_isConstant[soluteIndex]){
					...
					//System.out.println(">>>>>>>>>>>>>>>>."+_sIn[soluteIndex]);
	...
	public void updateBulk(SoluteGrid[] soluteGrid, SoluteGrid[] reacGrid, double timeStep) {
		...
		if (_bulkIsConstant)
		{
			//System.out.println("bulk constant updating..........");
			return;
		}
		//System.out.println("bulk not constant updating..........");
...
//			updateBulkByGradient(soluteGrid, timeStep);
		...
		if(Simulator.isChemostat){
			...
			//System.out.println("Not chemostat--------------------------------->");
			...
			for (int i=0; i<_bulkValue.length; i++)
				if (SimTimer.getCurrentTime()-_lastPulseTime[i] >= _pulseInterval[i]) {
					//System.out.println("pulsing......"+_pulseInterval[i]);
					// set bulk values to pulsed concentration
					//System.out.println("---------------->"+_bulkValue[i]);
					//MODIFIED BY HONEY
					//_bulkValue[i]=_bulkValue[i]+_sPulse[i];
					_bulkValue[i] = _bulkValue[i]+_sPulse[i];
					...
					if(_sPulse[i]>0)
						System.out.println("pulsing"+_sPulse[i] );
				}
			//commented by honey-->bulk constant
			//updateBulkByReaction(soluteGrid, timeStep);
		}
	}
...
//System.out.println("updateBulkByReaction-------------------<>"+timeStep);
		Domain aDomain;
		...
		for (int iGrid = 0; iGrid<reacGrid.length; iGrid++) {
			//System.out.println("iGrid"+iGrid);
			...
			if ( reacGrid[iGrid].gridName.contentEquals("pressure") )
				continue;
//COMMENTED BY HONEY --NEED TO FIX
			...
			//System.out.println("updateBulkByReaction-------------------1");
...
//			_bulkValue[iGrid] = _bulkValue[iGrid]+dSdT*timeStep;
		...
		LogFile.writeLog("Bulk update: "+message);
		//System.out.println("Bulk update: "+message);
	}
	...
	public void writeReport(ResultFile buffer) throws Exception {
		...
		// Concentration
		// bvm 23.01.09: added units to the output (ARE T
		// HEY CORRECT?)
		for (int i = 0; i<_bulkValue.length; i++) {
